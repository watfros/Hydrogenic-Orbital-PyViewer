...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "2D Polar plot of Y"))
        # self.label_3.setText(_translate("MainWindow", "Principal Quantum Number (n)："))
...
class RunThread(QThread):
    ...
    def __init__(self, l, m, planexyz, plt):
    ...
    def sph_harm_prof(self):
        ...
        A = np.sqrt(((2 * self.l + 1) * factorial(self.l - abs(self.m))) / (
                    4 * np.pi * factorial(self.l + abs(self.m))))  # Normalization constant
        # print("=====================================================")
        if self.planexyz == 0:
            theta = self.pi / 2
            ...
            ax1 = self.plt.fig.add_subplot(1, 1, 1)
            ax1.axis('equal')
        ...
        if self.planexyz == 1:  # 需要分phi = pi/2 or pi*3/2
            ...
            rho = calc_Y(theta,phi,self.l,self.m)
            # x,y,z = sph2cart(abs(rho),theta,phi)
            rholow = np.ma.masked_where(rho < 0.0, rho)  # 只保留rho>=0的情况，区分正负
            ...
            if tmp <= 0.01:
                ...
                y3 = 0.0 * y3
                z3 = 0.0 * z3
            rhoup = np.ma.masked_where(rho >= 0.0, rho)  # 只保留rho<0的情况，区分正负
        ...
        if self.planexyz == 2:  # 需要分phi = 0 or pi
            ...
            ax3.set_ylabel("Y_axis", fontsize=14, fontweight='bold')  # y轴上的名字
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def validationNLM(self):
        ...
        if s == True:
            print(l)
            print(m)
            RunThread(l, m, planexyz, self.cavas).run()
    def msg(self, msg):
