...
class RunThread(QThread):
    ...
    def run(self):
        if self.planexyz == 0:
            ...
            if plotax1 == False :
                f=0.0*f
            ax1.contour(x, y, f, levels=[-0.1 * limpsi, 0.1 * limpsi], colors=['blue', 'blue'])

            ax1.set_xlabel("X_axis")  # x轴上的名字
            ax1.set_ylabel("Y_axis")  # y轴上的名字
            ax1.set_xticks([])  # 关闭x刻度
            ax1.set_yticks([])  # 关闭y刻度
            ax1.set_title('profile of Psi_xy Squared', fontsize=14, fontweight='bold')
        ...
        if self.planexyz == 1:
            ...
            if plotax2 == False :
                f=0.0*f
            ax2.contour(y, z, f, levels=[-0.1 * limpsi, 0.1 * limpsi], colors=['blue', 'blue'])
            ...
            ax2.set_title('profile of Psi_yz Squared', fontsize=14, fontweight='bold')
        if self.planexyz == 2:
            ...
            if plotax3 == False :
                f=0.0*f
            ax3.contour(x, z, f, levels=[-0.1 * limpsi, 0.1 * limpsi], colors=['blue', 'blue'])
            ...
            ax3.set_title('profile of Psi_xz Squared', fontsize=14, fontweight='bold')
        ...
        if self.planexyz == 3:
            ...
            limpsi = np.maximum(maxpsi, minpsi)
            # ax3.contour(x,z,f,levels=[0.0], colors=['black'])
            ax3.contour(x, z, f, levels=[-0.1 * np.abs(limpsi), 0.1 * np.abs(limpsi)], colors=['blue', 'blue'])
