...
GRID_2D = 181      # Grid points for 2D figure
GRID_3D = 168      # Grid points for 3D figure
R_EXP = 1.65       # Radial range index, Rmax = 5 * n**R_EXP * a0
...
def associatedLaguerre(l, n, x):
    ...
    out = np.zeros_like(x) # output
    for m in range(len(coeffs)):
        out = out + coeffs[m]*(x**m)
    return out

#Calculate wave function value. Set Z=1 as the default parameter.
def calc_psi(r,Theta,Phi,n, l, m, Z=1):
    rho = 2.0*Z*r/n/a0
    ...
    #Normalization constant for radial part，with Z^3
    B = np.sqrt((2.0/n)**3 * Z**3 * factorial(n-l-1)/(2*n*factorial(n+l)**3))
    #Wave function normalization
    scalars = scalars * A * B
    return scalars

#Calculate wave function value and check if it is zero. Set Z=1 as the default parameter. 
def calc_psi_prof(r,Theta,Phi,n, l, m, Z=1):
    rho = 2.0*Z*r/n/a0
